...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="row stats-row">
            ...
            <div class="col-md-4">
                <div class="stat-card">
                    ...
                    <h4>{{ App\Models\CalonSiswa::whereIn('status', ['terbayar', 'verifikasi_keuangan', 'lulus'])->count() }}</h4>
            ...
            <div class="col-md-4">
                <div class="stat-card">
                    ...
                    <h4>Rp {{ number_format(collect($pembayaran ?? [])->sum('nominal_pembayaran'), 0, ',', '.') }}</h4>
        ...
        <div class="row" id="paymentList">
            ...
            <div class="col-md-6 payment-item" data-name="{{ strtolower($p->nama_lengkap ?? 'Test Student') }}" data-status="{{ $p->status }}" data-jurusan="{{ $p->jurusan_pilihan ?? 'PPLG' }}">
                    ...
                    <div class="card-body">
                        <div class="row align-items-center">
                            ...
                            <div class="col">
                                ...
                                <span class="badge bg-warning ms-1">{{ $p->status_label }}</span>
                            </div>
                        ...
                        <div class="row align-items-center">
                            <div class="col">
                                @if($p->bukti_pembayaran)
                                    <a href="{{ route('keuangan.file', ['id' => $p->id, 'type' => 'bukti_pembayaran']) }}" target="_blank" class="btn btn-view btn-sm me-2">
                            ...
                            <div class="col-auto">
                                @if($p->status == 'menunggu_pembayaran')
                                    <button class="btn btn-verify btn-sm me-2" onclick="verifyPayment({{ $p->id }})">
                                        <i class="fas fa-check"></i> Verifikasi
                                    </button>
                                    <button class="btn btn-reject btn-sm" onclick="rejectPayment({{ $p->id }})">
                                ...
                                @else
                                    <span class="badge bg-secondary">
                                        <i class="fas fa-info-circle"></i> {{ ucfirst($p->status) }}
                                    </span>
                                @endif
                            </div>
                        </div>
    ...
    <script>
        ...
        function verifyPayment(id) {
            if (confirm('Verifikasi pembayaran ini sebagai VALID?')) {
                const buttonContainer = document.querySelector(`[onclick="verifyPayment(${id})"]`).parentElement;
                
                fetch(`/keuangan/verifikasi/${id}`, {
                    method: 'PATCH',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        // Replace buttons with verified badge
                ...
                .catch(error => {
                    ...
                    showAlert('danger', 'Terjadi kesalahan saat verifikasi');
                });
            }
        }
        ...
        function confirmReject() {
            ...
            fetch(`/keuangan/verifikasi/${currentPaymentId}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const buttonContainer = document.querySelector(`[onclick="rejectPayment(${currentPaymentId})"]`).parentElement;
                    buttonContainer.innerHTML = `
            ...
            .catch(error => {
                console.error('Error:', error);
                showAlert('danger', 'Terjadi kesalahan saat menolak pembayaran');
            });
        }
