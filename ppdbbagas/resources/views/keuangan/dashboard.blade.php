                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between mb-2">
                            <span class="text-capitalize">{{ str_replace('_', ' ', $stat->jenis_pembayaran) }}</span>
                            <strong>Rp {{ number_format($stat->total, 0, ',', '.') }}</strong>
                        </div>
                        <div class="progress mb-3" style="height: 10px;">
                            <div class="progress-bar" style="width: {{ ($stat->total / $totalPendapatan) * 100 }}%"></div>
                        </div>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="payment-item d-flex justify-content-between align-items-center mb-3 p-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <strong class="d-block">{{ $bayar->pendaftar->dataSiswa->nama ?? 'Siswa Test' }}</strong>
                                    <small class="text-muted">{{ ucfirst($bayar->jenis_pembayaran ?? 'Pendaftaran') }}</small>
                                </div>
                            </div>
                            <div class="text-end">
                                <div class="fw-bold text-success">Rp {{ number_format($bayar->jumlah ?? 500000, 0, ',', '.') }}</div>
                                <span class="badge bg-{{ ($bayar->status ?? 'pending') == 'lunas' ? 'success' : (($bayar->status ?? 'pending') == 'pending' ? 'warning' : 'danger') }}">
                                    {{ ucfirst($bayar->status ?? 'Pending') }}
                                </span>
                            </div>
                        </div>
                        @empty
